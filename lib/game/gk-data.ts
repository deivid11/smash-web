...
/** ftDataGkoopa shares the ftKoopaAttributes layout (USA 1.02
 * ftKoopa_MotionState order, SpecialSHit bite variants sharing one figatree);
 * values come from PlGk.dat. Engine-authored orchestration reuses
 * [lib/game/koopa.ts](../../lib/game/koopa.ts); no WASM changes. */
export interface GkSpecialData {
  kind: 'Gk';
...
export function parseGkParameters(arc: HsdArchive): GkSpecialData {
  const base = arc.pointer(arc.symbol('ftDataGkoopa') + 4);
  const f = (offset: number) => { const v = arc.f32(base + offset); if (!Number.isFinite(v) || Math.abs(v) > 1000) throw Error('Invalid original Giga Bowser parameter.'); return v; };
  const u = (offset: number) => { const v = arc.u32(base + offset); if (v > 600) throw Error('Invalid original Giga Bowser integer.'); return v; };
  const p: GkSpecialData = {
    kind: 'Gk',
  ...
  if (p.breath.maxStrength <= p.breath.minStrength || p.breath.maxRange <= p.breath.minRange || p.breath.recoverStrength <= 0 ||
    ...
    throw Error('Unsupported original Giga Bowser bounds.');
  }
  return p;
}
/** ftGk subactions 246–266 (ftKoopa_MotionState order shifted by two; the two
 * SpecialSHit table entries are the bite variants Hit0/Hit1 sharing the
 * SpecialSHit figatree, verified against PlGk.dat). */
export const GK_ACTION_KEYS: ReadonlyArray<{ key: string; index: number; figatree: string }> = [
...
].map(([key, figatree], i) => ({ key: key!, index: 246 + i, figatree: figatree! }));
export const GK_MOVES: FighterContent['moves'] = {
...
/** ftDataGkoopa +0x48 slot 0: the Fire Breath flame item (itKoopaFlame layout,
 * same as Bowser's; Giga Bowser ships no EfGkData.dat and reuses EfKpData.dat). */
export function parseGkArticles(arc: HsdArchive): SpecialAssets['articles'] {
  const table = arc.pointer(arc.symbol('ftDataGkoopa') + 0x48);
  const article = arc.pointer(table), common = arc.pointer(article), special = arc.pointer(article + 4), states = arc.pointer(article + 12);
  if (!common || !special) throw Error('Original Giga Bowser flame article is incomplete.');
  const model = articleModel(arc, article, 'gkoopa-flame', states);
  const hit = arc.pointer(states + 12) ? itemHit(arc, arc.pointer(states + 12)) : null;
  if (!hit) throw Error('Missing original Giga Bowser flame hitbox.');
  ...
  // Same itKoopaFlame launch bounds as Bowser's: speed inside x8..xC, angle inside x10..x14.
