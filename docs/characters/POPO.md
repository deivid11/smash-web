...
| **Belay** — up | Solo-geometry native path: Start, Throw rise (`xA4` once, `xA8`/`xAC` fall after), Throw2, helpless. The partner-throw heights stay unported; with Nana active the rise is scaled ×1.6 as a labeled prototype stand-in for the partner throw (solo Popo keeps the native value exactly). Landing takes the original 25-frame lag. Ledge behavior is whatever the shared ledge system gives any helpless fighter. |
