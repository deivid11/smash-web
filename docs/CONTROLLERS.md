...
Smash attacks work three ways, all firing the same original smash data and charge (60-frame, original multiplier): hold the Strong button with a direction, flick the right stick (grounded side/up/down, directional aerials in air — hold it to charge, release to unleash), or flick the left stick fast while pressing Attack (A). The left-stick windows reuse the original PlCo stick-timing thresholds also used for item throws (side: full 0.8 tilt within 6 frames of the slam; up/down: past ±0.6625 within 4 frames), so held directions keep tilts, run keeps the dash attack, and crouch keeps the down tilt. The C-stick never moves the fighter, tap-jumps, fast-falls, drops through platforms or steers specials; it also counts for mash-outs and ledge attacks.
