...
- **P1:** A/D run, **Left Shift + A/D walk**, Space jump, W/S aim, S drop/fast-fall, J/K attacks, **L special**, **U shield/air dodge**, **I grab**, **T taunt**.
- **P2:** arrows move/aim/drop, **slash + arrows walk**, **Enter jump**, N/M attacks, **comma special**, **Right Shift shield/air dodge**, **period grab**, **B (or numpad 3) taunt**.
- Every key above is rebindable in **Options → Keyboard**; on a standard controller the taunt sits on **View / Select** and can be remapped like any other button.
...
- **Taunt** plays each fighter's own original Appeal motion, with its voice cue: standing on the ground only, committal until the clip ends, and cut short only by a hit. Fighters authored with a left/right or raised pair taunt with the clip matching their facing. See [docs/TAUNTS.md](docs/TAUNTS.md).
