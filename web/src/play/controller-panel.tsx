  ...
  return <article className="controller-panel__device" aria-label={`Controller ${device.index + 1}`} data-controller-index={device.index}>
    ...
    {device.connected && <>
      ...
      {calibration ? <fieldset className="controller-panel__calibration"><legend>Remap controller · input disabled until saved or cancelled</legend>
        <p>Center every stick, then Listen + move RIGHT or UP per axis. Bind every action; Walk and Taunt are optional.</p>
        ...
        {ACTIONS.map(action => <div className="controller-panel__binding" key={action}><strong>{ACTION_LABELS[action]}{action === 'walk' || action === 'taunt' ? ' · optional' : ''}</strong><span>{calibration.mapping.buttons[action].map(label).join(' / ') || 'Unbound'}</span>
      ...
      </fieldset> : device.mapping && <details className="controller-panel__bindings"><summary>Current gameplay mapping</summary><dl>{ACTIONS.map(action => <div key={action}><dt>{ACTION_LABELS[action]}</dt><dd>{device.mapping!.buttons[action].map(label).join(' / ') || (action === 'walk' || action === 'taunt' ? 'Optional / keyboard' : 'Stick axis / not bound')}</dd></div>)}<div><dt>Smash-stick horizontal</dt><dd>{device.mapping!.axes.cx !== null && device.mapping!.axes.cx !== undefined ? `Axis ${device.mapping!.axes.cx!.index}` : 'Unbound'}</dd></div><div><dt>Smash-stick vertical</dt><dd>{device.mapping!.axes.cy !== null && device.mapping!.axes.cy !== undefined ? `Axis ${device.mapping!.axes.cy!.index}` : 'Unbound'}</dd></div></dl></details>}
    </>}
